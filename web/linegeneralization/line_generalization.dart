...
List smoothMcMaster(points) {
  ...
  while (i-- > 0) {
    if (i == len - 1 || i == len - 2 || i == 1 || i == 0) {
      ...
      avX = avX / 5;
      avY = avY / 5;
      nL[i] = nL[i] = new Math.Point((points[i].x + avX) / 2, (points[i].y + avY) / 2);
    }
  }
  return nL;
}
...
List simplifyLang(lookAhead, tolerance, points) {
  ...
  var nP = new List();
  nP.add( new Math.Point(points[0].x, points[0].y) );
  for (var i = 0; i < len; i++) {
    if (i + lookAhead >= len) {
      lookAhead = len - i - 1;
    }
    ...
    if (offset > 0) {
      nP.add( new Math.Point(points[i + offset].x, points[i + offset].y) );
...
num recursiveToleranceBar( points, i, lookAhead, tolerance){
  ...
  v1 = new Math.Point(points[i + n].x - cP.x,points[i + n].y - cP.y);
  ...
  for (var j = 1; j <= n; j++) {
    ...
    v2 = new Math.Point( cLP.x - cP.x,cLP.y - cP.y);
